...
# dev = [('Find the birth date of the actor who played roles in First Wives Club and Searching for the Elephant.',
#         ['February 13, 1980'])]
...
tools = [
    Tool(
        name="Search",
        func=search.run,
        description="Use this tool to do a google search for information on the internet only when needed. All google search syntax is available."
    )
]
...
Write a search query that will help to answer a complex question.
When writing queries, you can use the Google search syntax. 
For example, when searching for a character name that appears in both Harry Potter and Silence of the Lambs, 
your query could look like this: character +'Harry Potter' +'Silence of the Lambs'.
---
...
followup_hop_chain = LLMChain(llm=llm, prompt=followup_hop_prompt)  # set verbose=True to see search results
...
with get_openai_callback() as cb:
    ...
    print("\n\nThe cost of this run was:", cb.total_tokens, " tokens.")
